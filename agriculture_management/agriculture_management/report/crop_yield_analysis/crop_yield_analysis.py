...
def get_data(filters):
	# FIX: Removed f-string. _build_conditions returns only column names and
	# %(key)s placeholders — never raw user values — so string-joining the
	# WHERE clauses is safe. Actual values are bound via the `filters` dict
	# passed as the second argument to frappe.db.sql.
...
def grafik(filters):
	# FIX: Same approach — conditions string contains only column refs and
	# %(key)s placeholders; user values are bound via the filters dict.
	conditions = _build_conditions(filters)
